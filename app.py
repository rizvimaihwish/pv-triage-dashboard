...
<body onload="initApp()">
    ...
    <!-- All The Beautiful Floating Doodles -->
    ...
    <!-- WINDOW 5: ENHANCED MENSTRUAL TRACKER -->
    ...
    <script>
        ...
        function jumpTo(winId) {
            ...
            if(winId === 5) calculateCyclePhase(); 
        }
        ...
        function calculateCyclePhase() {
            ...
            let lmpStr = document.getElementById('p_lmp').value;
            if(!lmpStr) return;
            
            let lmp = new Date(lmpStr);
            let today = new Date();
            let cycle = parseInt(document.getElementById('p_cycle_tracker').value) || 28;
            
            let nextPeriod = new Date(lmp);
            ...
            document.getElementById('pred_next').innerText = nextPeriod.toLocaleDateString('en-US', options);
            document.getElementById('pred_ovulation').innerText = ovulation.toLocaleDateString('en-US', options);
            document.getElementById('pred_fertile').innerText = fertileStart.toLocaleDateString('en-US', options);
            
            let diffTime = today - lmp;
            let diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
            
            if(diffDays < 0) diffDays = 0; 
            let dayOfCycle = (diffDays % cycle) + 1;

            let phase = ""; let advice = "";
            let bM = document.getElementById('bar-menstrual'), bF = document.getElementById('bar-follicular'), bO = document.getElementById('bar-ovulation'), bL = document.getElementById('bar-luteal');
            
            bM.style.opacity = "0.3"; bF.style.opacity = "0.3"; bO.style.opacity = "0.3"; bL.style.opacity = "0.3";

            if(dayOfCycle >= 1 && dayOfCycle <= 5) {
                phase = "Menstruation"; bM.style.opacity = "1";
                advice = "AI Suggestion: Prioritize rest, iron-rich foods, and light walking.";
            } else if(dayOfCycle >= 6 && dayOfCycle <= 13) {
                phase = "Follicular"; bF.style.opacity = "1";
                advice = "AI Suggestion: Estrogen is rising. High energy for heavy workouts!";
            } else if(dayOfCycle >= 14 && dayOfCycle <= 16) {
                phase = "Ovulation"; bO.style.opacity = "1";
                advice = "AI Suggestion: Peak strength! Maintain hydration.";
            } else {
                phase = "Luteal Phase"; bL.style.opacity = "1";
                advice = "AI Suggestion: Progesterone rise may cause PMS. Increase Magnesium (Dark Chocolate).";
            }

            document.getElementById('cycle_phase').innerText = `Day ${dayOfCycle} - ${phase}`;
            document.getElementById('cycle_advice').innerText = advice;
        }

        function savePeriodToHistory() {
            let lmpStr = document.getElementById('p_lmp').value;
            let cycleLen = document.getElementById('p_cycle_tracker').value;
            if(!lmpStr) return;
            
            let dateObj = new Date(lmpStr);
            let formattedDate = dateObj.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
            
            if(cycleHistory.includes(formattedDate)) {
                alert("This cycle is already logged.");
                return;
            }
            
            cycleHistory.unshift(formattedDate); 
            
            let listHTML = "";
            cycleHistory.forEach((date, index) => {
                listHTML += `<div class="history-item"><span>Cycle ${cycleHistory.length - index}</span><span style="color:var(--deep-magenta);">${date}</span></div>`;
            });
            
            document.getElementById('cycle_history_list').innerHTML = listHTML;
            alert("Cycle data safely logged to history.");
        }
